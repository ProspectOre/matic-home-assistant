...
- `select`: cleaning mode, coverage, and selected saved cleaning plan.
- `button`: run the selected plan, intelligent-rotation override,
...
- `binary_sensor`: cleaning, paused, returning, charging, low charge, fully
  ...
  session, SSH tunnel permission, and diagnostic upload.
...
- `matic_robot.run_selected_plan`: run the selected or named plan using its
...
The hash-only default keeps the action safe for inspection and automation
without exposing raw collection bytes.
...
`hermes_error_codes` while also exposing official snake-case `errors` and
