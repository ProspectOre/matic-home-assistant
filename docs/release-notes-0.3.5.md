...
- If Matic publishes one new, matching native single-room completion within
  ...
  history and duration. Credit requires the exact original dispatch identity,
  so a stale watcher cannot claim a newer run of the same room. Expired markers
  ...
  When the pre-clean history baseline is unavailable, completion remains
  deliberately uncredited but an expiry-only watcher still clears the marker.
...
The release candidate passes 955 Python tests / 9,014 statements at 100%
