...
def test_python_package_includes_home_assistant_runtime_files() -> None:
    ...
    assert "brand/*.png" not in package_data
...
def test_integration_ships_no_brand_directory() -> None:
    """Keep brand artwork out of the packaged integration."""
    assert not (INTEGRATION / "brand").exists()
