...
class MaticMapPanel extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: "open" });
    this._view = "photo";
  ...
  _entities() {
    ...
    return {
      photo: states.find(([, state]) => state.attributes?.source === "local_robot_slam"),
      rooms: states.find(([, state]) => state.attributes?.robot_location_source),
    };
  }
  ...
  _zoom(factor, centerX, centerY) {
    ...
    this._scale = Math.max(1, Math.min(10, previous * factor));
    const ratio = this._scale / previous;
    this._offsetX = x - rect.left - (x - rect.left - this._offsetX) * ratio;
    this._offsetY = y - rect.top - (y - rect.top - this._offsetY) * ratio;
    this._applyTransform();
  }
  ...
  _applyTransform() {
    ...
    if (this._scale <= 1) {
      ...
      this._offsetX = Math.max(
        width - width * this._scale - marginX,
        Math.min(marginX, this._offsetX),
      );
      this._offsetY = Math.max(
        height - height * this._scale - marginY,
        Math.min(marginY, this._offsetY),
      );
    }
    image.style.transform = `translate(${this._offsetX}px, ${this._offsetY}px) scale(${this._scale})`;
    this.shadowRoot.querySelector(".zoom-value").textContent = `${Math.round(this._scale * 100)}%`;
  }

  _update(force = false) {
    const entities = this._entities();
    let selected = entities[this._view];
    if (!selected && this._view === "photo") selected = entities.rooms;
    ...
    if (!selected) {
      ...
      this.shadowRoot.querySelector(".status").textContent =
        "No local map camera is available yet";
      return;
    }
  ...
  _handleKeyDown(event) {
    ...
    if (event.key === "+" || event.key === "=") {
      ...
      if (event.key === "ArrowLeft") this._offsetX += amount;
      if (event.key === "ArrowRight") this._offsetX -= amount;
      if (event.key === "ArrowUp") this._offsetY += amount;
      if (event.key === "ArrowDown") this._offsetY -= amount;
      this._applyTransform();
    }
  }
  ...
  _render() {
    this.shadowRoot.innerHTML = `
      <style>
        :host { display: block; height: 100%; min-height: 0; color: var(--primary-text-color); background: #0b1118; }
        .shell { height: 100dvh; min-height: 480px; display: grid; grid-template-rows: auto minmax(0, 1fr); overflow: hidden; background: radial-gradient(circle at 50% 20%, #172334, #0b1118 65%); }
        header { display: flex; flex-wrap: wrap; gap: 10px; align-items: center; padding: 14px 18px; background: color-mix(in srgb, var(--card-background-color) 92%, transparent); border-bottom: 1px solid var(--divider-color); z-index: 2; }
        .heading { display: grid; gap: 2px; margin: 0 14px 0 0; }
        h1 { margin: 0; font-size: 20px; }
        .privacy { color: var(--secondary-text-color); font-size: 11px; }
        .spacer { flex: 1 1 auto; }
        button, a { min-height: 46px; box-sizing: border-box; display: inline-flex; align-items: center; justify-content: center; padding: 0 16px; border: 1px solid var(--divider-color); border-radius: 12px; color: var(--primary-text-color); background: var(--card-background-color); text-decoration: none; cursor: pointer; touch-action: manipulation; }
        ...
        .zoom-value, .status { color: var(--secondary-text-color); font-variant-numeric: tabular-nums; }
        .viewport { position: relative; min-height: 0; overflow: hidden; touch-action: none; cursor: grab; outline: none; }
        .viewport:focus-visible { box-shadow: inset 0 0 0 2px var(--primary-color); }
        .viewport.dragging { cursor: grabbing; }
        .map-image { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: contain; transform-origin: 0 0; will-change: transform; user-select: none; -webkit-user-drag: none; }
        .viewport.loading::after { content: ""; position: absolute; top: 14px; right: 14px; width: 22px; height: 22px; border: 3px solid rgba(255,255,255,.25); border-top-color: var(--primary-color); border-radius: 50%; animation: spin .8s linear infinite; }
        @keyframes spin { to { transform: rotate(360deg); } }
        .empty { position: absolute; inset: 0; display: grid; place-items: center; color: var(--secondary-text-color); font-size: 18px; text-align: center; padding: 40px; }
        .empty[hidden] { display: none; }
        @media (max-width: 720px) { .heading { width: 100%; } .status { order: 10; width: 100%; } header { padding: 8px; gap: 6px; } button, a { min-height: 42px; padding: 0 10px; } }
      </style>
      <div class="shell">
        <header>
          <div class="heading"><h1>Matic map studio</h1><span class="privacy">Live from your robot · stays inside Home Assistant</span></div>
          <button data-view="photo" class="selected" aria-pressed="true">3D photo</button>
          <button data-view="rooms" aria-pressed="false">Rooms</button>
          <span class="status">Loading local map…</span>
          <span class="spacer"></span>
        ...
        <div class="viewport" tabindex="0" role="application" aria-label="Interactive local Matic map. Drag to pan; use the mouse wheel or plus and minus keys to zoom.">
          <img class="map-image" alt="Matic local map" draggable="false">
          <div class="empty">The local map will appear when the Matic integration is ready.</div>
        </div>
      </div>
    `;
    this._guardButton(this.shadowRoot.querySelector('[data-view="photo"]'), () => this._setView("photo"));
    this._guardButton(this.shadowRoot.querySelector('[data-view="rooms"]'), () => this._setView("rooms"));
    this._guardButton(this.shadowRoot.querySelector(".zoom-out"), () => this._zoom(1 / 1.2));
    ...
    viewport.addEventListener("dblclick", (event) => {
      event.preventDefault();
      this._zoom(1.5, event.clientX, event.clientY);
    });
    const image = this.shadowRoot.querySelector(".map-image");
...
if (!customElements.get("matic-map-panel-v0-3-0")) {
  customElements.define("matic-map-panel-v0-3-0", MaticMapPanel);
}
