...
`]}#C;disconnectedCallback(){this.#C!==void 0&&clearTimeout(this.#C),this.#C=void 0,super.disconnectedCallback()}#H(H,V,e){return T(this.localize,H,V,e)}#V(H){return H==="vacuum"?this.#H("vacuum","Vacuum"):H==="mop"?this.#H("mop","Mop"):this.#H("vacuum_and_mop","Vacuum + mop")}#r(H){return H==="quick"?this.#H("quick","Quick"):H==="standard"?this.#H("standard","Optimal"):this.#H("heavy_duty","Heavy Duty")}#t(H){this.dispatchEvent(new CustomEvent(H1,{detail:H,bubbles:!0,composed:!0}))}#L(){return this.state.notice?d`
    ...
    `}#h(H){let V=this.state.planDraft.rooms,r=V.find(t=>t.roomId===H)?V.filter(t=>t.roomId!==H):[...V,{roomId:H,cleaningMode:"vacuum",coverageSetting:"standard"}];this.#t({type:"patch-plan-draft",patch:{rooms:r}})}#f(H,V){let e=this.state.planDraft.rooms.map((r,t)=>t===H?{...r,...V}:r);this.#t({type:"patch-plan-draft",patch:{rooms:e}})}#u(H,V){let e=H+V,r=[...this.state.planDraft.rooms];if(e<0||e>=r.length)return;let[t]=r.splice(H,1);t&&(r.splice(e,0,t),this.#t({type:"patch-plan-draft",patch:{rooms:r}}))}#d(){let H=this.state.resources.plans,V=H.value,e=this.state.planDraft,r=e.rooms.map(M=>({room:M,label:V?.rooms.find(i=>i.roomId===M.roomId)?.name||"Room",selected:!0})),t=(V?.rooms||[]).filter(M=>!e.rooms.some(i=>i.roomId===M.roomId)).map(M=>({room:{roomId:M.roomId,cleaningMode:"vacuum",coverageSetting:"standard"},label:M.name,selected:!1})),o=[...r,...t];return this.#i(H.status,H.problem,d`
      <div class="stack">
        <div class="split">
          ...
          <button class="ms-btn ms-btn--secondary" type="button" @click=${()=>this.#t({type:"select-plan",planId:null})}>${Z(D3)}<span class="ms-btn__label">${this.#H("plan_new","New plan")}</span></button>
        </div>
              ...
              <div class="room plan-room ms-row ms-row--stack" data-selected=${String(a)}>
                ...
                ${a?d`
                  <div class="split room-settings">
                    ...
                    <label class="field ms-field">${this.#H("cleaning_mode","Cleaning mode")}
                      <select aria-label=${this.#H("v4_room_cleaning_mode_named","Cleaning mode for {room}",{room:i})} .value=${M.coverageSetting} @change=${A=>this.#f(n,{coverageSetting:I(A)})}>${K2.map(A=>d`<option value=${A} ?selected=${A===M.coverageSetting}>${this.#r(A)}</option>`)}</select>
                    </label>
                  </div>
                `:l}
              </div>
        ...
        <div class="plan-options" role="group" aria-labelledby="completion-heading">
          <label class="plan-option">
            <input type="checkbox" .checked=${e.returnToBase} @change=${M=>this.#t({type:"patch-plan-draft",patch:{returnToBase:e0(M)}})}>
          ...
          <label class="plan-option">
            <input type="checkbox" .checked=${e.finishCurrentRoom} @change=${M=>this.#t({type:"patch-plan-draft",patch:{finishCurrentRoom:e0(M)}})}>
            <span class="plan-option-copy">
    ...
    `}#p(){let H=this.state.areaDraft,V=H.canRebind||H.status==="review",e=H.status==="stale"||H.status==="unknown";return d`
      <div class="stack">
        ...
        <div class="split">
          <label class="field ms-field">${this.#H("v4_cleaning_system","Cleaning system")}
            <select .value=${H.cleaningMode} @change=${r=>this.#t({type:"patch-area-draft",patch:{cleaningMode:I(r)}})}>${q2.map(r=>d`<option value=${r} ?selected=${r===H.cleaningMode}>${this.#V(r)}</option>`)}</select>
          </label>
          <label class="field ms-field">${this.#H("cleaning_mode","Cleaning mode")}
            <select .value=${H.coverageSetting} @change=${r=>this.#t({type:"patch-area-draft",patch:{coverageSetting:I(r)}})}>${K2.map(r=>d`<option value=${r} ?selected=${r===H.coverageSetting}>${this.#r(r)}</option>`)}</select>
          </label>
        </div>
