      ...
      <div class="map-rail" data-map-control>
        ...
        ${a?p`
          <div class="camera-steps ms-surface ms-surface--floating ms-segment" role="toolbar" aria-orientation="horizontal" aria-label=${this.#M("map_camera_controls","Map camera controls")}>
            <button class="ms-btn ms-btn--icon" type="button" aria-label=${this.#M("map_rotate_left","Rotate left")} aria-keyshortcuts="[" @click=${()=>this.#d(-52,0)}>${S(j3)}</button>
            <button class="ms-btn ms-btn--icon" type="button" aria-label=${this.#M("map_tilt_down","Lower viewing angle")} aria-keyshortcuts="PageDown" @click=${()=>this.#d(0,30)}>${S(K3)}</button>
            <button class="ms-btn ms-btn--icon" type="button" aria-label=${this.#M("map_tilt_up","Raise viewing angle")} aria-keyshortcuts="PageUp" @click=${()=>this.#d(0,-30)}>${S(G3)}</button>
            <button class="ms-btn ms-btn--icon" type="button" aria-label=${this.#M("map_rotate_right","Rotate right")} aria-keyshortcuts="]" @click=${()=>this.#d(52,0)}>${S(Y3)}</button>
          </div>
        `:s}
        ...
        ${n?p`
          <div class="map-tools ms-surface ms-surface--floating ms-segment" role="group" aria-label=${this.#M("v4_map_tools","Map tools")}>
            ${V?s:p`
              ...
              >${S(Q3)}<span class="ms-btn__label">${this.#M("v4_fit_map","Fit map")}</span></button>
            `}
          </div>
        `:s}
        ${!V&&A?p`
          <div class="map-extras ms-surface ms-surface--floating ms-segment" role="group" aria-label=${this.#M("v4_map_display","Map display")}>
              ...
              >${S(q3)}<span class="ms-btn__label">${this.#M("v4_room_names","Room names")}</span></button>
              <button
...
`),e=typeof navigator>"u"?void 0:navigator.clipboard;if(e&&typeof e.writeText=="function")try{await e.writeText(V),this.#t("copied");return}catch{}this.#t(this.#x(V,H instanceof HTMLElement?H:null)?"copied":"failed")}#d(){let H=this.#S(),V=this._copyStatus==="copied"?this.#H("v4_copied","Copied"):this._copyStatus==="failed"?this.#H("v4_copy_failed","The summary could not be copied. Select the text to copy it by hand."):"";return p`
    ...
    `}#L1(H,V){let e=(f,O,y)=>this.#C(f,O,y),r=this.#z(e("v4_map_history","Map history"),a5,()=>this.#l("history"),e("v4_map_history_detail","Saved maps are floor-scoped and read only.")),t=this.#z(e("v4_map_diagnostics","Map diagnostics"),i0,()=>this.#l("support"),e("v4_map_support_detail","Private geometry is never included.")),{host:M}=H;if(!M.connected)return this.#W(e("v4_reconnecting_title","Reconnecting to Home Assistant"),e("v4_reconnecting_body","The last verified map stays read-only until the connection returns."));if(!M.administrator)return this.#W(e("v4_admin_title","Administrator access required"),e("v4_admin_body","Ask a Home Assistant administrator to open this map."));if(M.robotCount===0)return this.#W(e("v4_no_robot_title","No Matic robot set up"),e("v4_no_robot_body","Add the Matic integration to see a map here."),x`<a class="ms-btn ms-btn--secondary" href="/config/integrations/integration/matic_robot">${e("v4_open_integration","Open the Matic integration")}</a>`);if(!M.robotConnected)return x`
      ...
      <h3 class="shelf-heading">${e("v4_more","Map tools")}</h3>
      <div class="shelf">
        ${this.#z(e("v4_custom_areas","Clean a custom area"),A2,()=>this.#l("draw"),Z,o)}
        ${r}
    ...
    `}render(){let H=this.state,V=H.narrowHint||this._measuredNarrow,e=J7(H,this.localize),r=U2({...H,narrowHint:V}),t=B5(H),M=!V&&r.id==="stop"?r:!V&&t?.id==="stop"?t:null,o=M&&M===r?null:r,i=H.workflow==="draw"&&H.dataMode==="live"?{id:"clear-draft",label:"Clear drawing",labelKey:"v4_clear_drawing",kind:"neutral",enabled:H.draw.circles.length>0||!!H.draw.outline?.points.length}:null,a=M&&M===t?null:t??i,n=H.fullMap&&(H.coherence==="verifying"||H.coherence==="booting"),A=H.fullMap||H.host.administrator&&H.host.robotCount>0&&H.map.available,l=r4(H.dialog,this.localize,H.workflow==="plan"),d=V&&!H.fullMap?`--map-sheet-offset:${this._sheetOffset}px`:"--map-sheet-offset:0px",m=V&&H.workflow==="draw",u=H.precisionOpen&&H.workflow==="draw";return x`
              ...
              ${V?x`
                ...
                ${m?x`
                  <div class="sheet-tools">
                    ${s2(H,{intent:h=>this.#n(h),openBrush:()=>this.#_(),t:(h,Z)=>this.#C(h,Z)},"grid")}
                    ${u?x`
                      <div class="precision-popover">
