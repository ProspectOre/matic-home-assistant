...
`]}#i;disconnectedCallback(){super.disconnectedCallback()}#e(e,o,t){return O(this.localize,e,o,t)}#o(e){return e==="vacuum"?this.#e("vacuum","Vacuum"):e==="mop"?this.#e("mop","Mop"):this.#e("vacuum_and_mop","Vacuum + mop")}#a(e){return e==="quick"?this.#e("quick","Quick"):e==="standard"?this.#e("standard","Optimal"):this.#e("heavy_duty","Heavy Duty")}#c(e){return e==="mop_due"?this.#e("v4_cadence_mop_due_reason","Vacuum and mop are due"):e==="coverage_due"?this.#e("v4_cadence_coverage_due_reason","Periodic coverage is due"):this.#e("v4_cadence_due_unknown_reason","Schedule setting due")}#g(e){return e==="plan_disabled"?this.#e("v4_preview_plan_disabled","This plan is paused. Enable it to preview and run."):e==="shared_schedule_unavailable"?this.#e("v4_preview_shared_unavailable","The shared room schedule cannot be verified on this map."):e==="cadence_identity_changed"||e==="cadence_identity_unavailable"?this.#e("v4_preview_identity_unavailable","Room identity could not be verified. Check the current map before running."):e==="plan_has_no_rooms"?this.#e("v4_preview_no_rooms","Add at least one room to this plan."):this.#e("v4_preview_invalid","The saved plan preview is invalid. Review the plan and try again.")}#t(e){this.dispatchEvent(new CustomEvent(j,{detail:e,bubbles:!0,composed:!0}))}#r(){return this.state.notice?n`
    ...
    `:p}#_(){switch(this.state.workflow){case"rooms":case"plans":case"plan":return{loading:this.#e("v4_loading_rooms_plans","Loading rooms and plans\u2026"),unavailable:this.#e("v4_rooms_plans_unavailable","Rooms and plans are unavailable right now."),empty:this.#e("v4_no_rooms_plans","No rooms or plans are available yet.")};case"draw":case"areaReview":return{loading:this.#e("v4_loading_areas","Loading saved areas\u2026"),unavailable:this.#e("v4_areas_unavailable","Saved areas are unavailable right now."),empty:this.#e("v4_no_saved_areas","No saved areas yet. Draw one on the map.")};case"history":return{loading:this.#e("v4_loading_history","Loading map history\u2026"),unavailable:this.#e("v4_history_unavailable","Map history is unavailable right now."),empty:this.#e("v4_no_map_history","No saved map snapshots yet.")};default:return{loading:this.#e("map_loading","Loading\u2026"),unavailable:this.#e("v4_workspace_unavailable","This workspace is unavailable right now."),empty:this.#e("v4_nothing_saved","Nothing saved yet.")}}}#n(e,o,t){let a=this.#_();if(e==="loading"||e==="idle")return n`<div class="loading" role="status">${a.loading}</div>`;if(e==="error"){let s=this.state.workflow;return n`
      ...
      `}return e==="empty"?n`<div class="empty">${a.empty}</div>`:t}#b(){let e=this.state.resources.plans,o=M(this.state),t=o!==null;return this.#n(e.status,e.problem,n`
      <div class="stack">
        ...
        <div class="list" role="group" aria-labelledby="rooms-heading">
          ${(e.value?.rooms||[]).map(a=>{let s=this.state.selection.roomIds.includes(a.roomId);return n`
              <div class="room ms-row ms-row--stack" data-selected=${String(s)}>
                <label class="room-choice">
                  ...
                  ${s?n`<small>${this.#e("v4_room_ready","Ready")}</small>`:p}
                </label>
        ...
        <div class="stack" aria-label=${this.#e("v4_shared_schedule_status","Shared room schedule status")}>
          ${this.state.selection.roomIds.map(a=>{let s=e.value?.rooms.find(l=>l.roomId===a);return s?.sharedCadence||s?.sharedCadenceProgress||s?.sharedCadenceReasons?.length?n`<p class="subtle">${s.name}: ${this.#m(s.sharedCadence,s.sharedCadenceProgress,s.sharedCadenceReasons)}</p>`:p})}
        </div>
        ${this.state.manualRoomPreview.status==="loading"?n`<p role="status" class="subtle">${this.#e("v4_room_preview_loading","Verifying the effective room settings\u2026")}</p>`:p}
        ${this.state.manualRoomPreview.status==="error"?n`<div class="stack">
    ...
    `)}#f(e){let o=new Set(e.missionBoundaries);return n`
    ...
    `}#$(e,o){let t=this.state.resources.plans.value?.rooms.find(a=>a.roomId===e)?.name||this.#e("v4_room","Room");return n`
      <div class="split room-settings">
        <label class="field ms-field">${this.#e("v4_cleaning_system","Cleaning system")}
          ...
          >${I.map(a=>n`<option value=${a} ?selected=${a===o.cleaningMode}>${this.#o(a)}</option>`)}</select>
        </label>
    ...
    `}#d(e){return e.cadence||{scope:"plan",mopEveryN:null,coverageEveryN:null,periodicCoverageSetting:null,doMopNext:!1,doCoverageNext:!1}}#m(e,o,t=o?.reasons||[]){if(t.includes("identity_changed")||t.includes("room_not_on_current_map"))return this.#e("v4_cadence_identity_blocked","Schedule identity does not match the current room map. Review this room before cleaning.");if(t.includes("shared_schedule_unavailable"))return this.#e("v4_cadence_shared_unavailable","The shared room schedule is unavailable. Review its settings before cleaning.");if(t.includes("invalid_cadence_policy"))return this.#e("v4_cadence_policy_invalid","The room schedule needs review before it can be applied.");if(!e||!e.mopEveryN&&!e.coverageEveryN)return this.#e("v4_cadence_not_enabled","No recurring room schedule is enabled.");let a=[],s=o?.mopDue||o?.reasons.includes("mop_due"),l=o?.coverageDue||o?.reasons.includes("coverage_due");return e.mopEveryN&&a.push(s?this.#e("v4_cadence_mop_due","Vacuum and mop is due on this clean."):this.#e("v4_cadence_mop_progress","Mop every {interval} cleans; {completed} qualifying cleans since the last mop.",{interval:e.mopEveryN,completed:o?.mopProgress??0})),e.coverageEveryN&&a.push(l?this.#e("v4_cadence_coverage_due","{coverage} periodic coverage is due on this clean.",{coverage:this.#a(e.periodicCoverageSetting||"standard")}):this.#e("v4_cadence_coverage_progress","Periodic coverage every {interval} cleans; {completed} qualifying cleans so far.",{interval:e.coverageEveryN,completed:o?.coverageProgress??0})),a.join(" ")}#s(e,o){let t=this.state.planDraft.rooms[e];t&&this.#l(e,{cadence:{...this.#d(t),...o}})}#p(e,o,t){let a=e.currentTarget,s=a.value;if(s===""){this.#s(o,{[t]:null});return}let l=Number(s);if(!Number.isInteger(l)||l<1||l>100){a.reportValidity();return}let u=this.state.planDraft.rooms[o];if(u){if(t==="coverageEveryN"){this.#s(o,{coverageEveryN:l,periodicCoverageSetting:u.cadence?.periodicCoverageSetting||u.coverageSetting});return}this.#s(o,{[t]:l})}}#y(e,o,t){let a=this.state.planDraft,s=this.#d(e),l=e.cadenceProgress,h=(a.id?this.state.resources.plans.value?.plans.find(_=>_.id===a.id)?.rooms.find(_=>_.roomId===e.roomId):void 0)?.cadence?.scope??null,b=this.state.resources.plans.value?.rooms.find(_=>_.roomId===e.roomId),i=!!(b?.sharedCadence||b?.sharedCadenceProgress),c=!!(s.mopEveryN||s.coverageEveryN),d=!!(l?.mopProgress||s.doMopNext),v=!!(l?.coverageProgress||s.doCoverageNext),m=a.dirty||this.state.command!=="idle"||this.state.managedLock||this.state.activity!=="idle"&&this.state.activity!=="docked"||this.state.dataMode!=="live",w=h!==null&&h!==s.scope?s.scope==="shared"?this.#e("v4_cadence_shared_join_effect","Saving makes this schedule shared across participating plans and opted-in room cleans. It adopts existing shared progress when available; a new shared schedule starts at zero."):this.#e("v4_cadence_shared_leave_effect","Saving starts fresh private progress for this plan. The existing shared schedule remains unchanged for other participating plans."):h===null&&c?s.scope==="shared"?i?this.#e("v4_cadence_shared_join_effect","Saving makes this schedule shared across participating plans and opted-in room cleans. It adopts existing shared progress when available; a new shared schedule starts at zero."):this.#e("v4_cadence_new_shared_effect","Saving creates a shared schedule for participating plans and opted-in room cleans. Its progress starts at zero."):this.#e("v4_cadence_new_private_effect","Saving creates private progress for this plan only. It starts at zero when enabled."):this.#e("v4_cadence_existing_effect","Interval edits keep progress, and disabling pauses it. Mopping and coverage have separate progress and resets."),x=this.#e("v4_room_cadence_named","Room schedule for {room}",{room:t});return n`
      <details class="plan-option cadence-config">
        ...
        <div class="split room-settings">
          ...
          ${s.coverageEveryN?n`
            <label class="field ms-field">${this.#e("v4_cadence_periodic_coverage","Periodic coverage setting")}
              <select aria-label=${this.#e("v4_cadence_periodic_coverage_named","Periodic coverage setting for {room}",{room:t})} .value=${s.periodicCoverageSetting||"standard"} @change=${_=>this.#s(o,{periodicCoverageSetting:f(_)})}>${E.map(_=>n`<option value=${_} ?selected=${_===s.periodicCoverageSetting}>${this.#a(_)}</option>`)}</select>
            </label>
          `:p}
        </div>
        <p class="subtle" role="status">${w}</p>
        ${e.cleaningMode!=="vacuum"?n`<p class="subtle">${this.#e("v4_cadence_mop_requires_vacuum","Set this room's normal cleaning system to vacuum to enable periodic mopping.")}</p>`:p}
        ${s.mopEveryN&&e.cleaningMode==="vacuum"?n`<p class="subtle">${this.#e("v4_cadence_clear_mop_to_change_normal","Clear the mopping interval before changing this room's normal cleaning system.")}</p>`:p}
        ...
        ${a.id&&(s.mopEveryN||s.coverageEveryN||d||v)?n`
          <div class="toolbar">
            ${s.mopEveryN||d?n`<button
    ...
    `)}#x(){let e=this.state.resources.plans,o=e.value,t=this.state.planDraft,a=t.rooms.map(i=>({room:i,label:o?.rooms.find(c=>c.roomId===i.roomId)?.name||"Room",selected:!0})),s=(o?.rooms||[]).filter(i=>!t.rooms.some(c=>c.roomId===i.roomId)).map(i=>({room:{roomId:i.roomId,cleaningMode:"vacuum",coverageSetting:"standard"},label:i.name,selected:!1})),l=[...a,...s],u=new Set(t.rooms.map(i=>`${i.cleaningMode}:${i.coverageSetting}`)).size>1,b=(t.id?o?.plans.find(i=>i.id===t.id):void 0)?.nextRunPreview;return this.#n(e.status,e.problem,n`
      <div class="stack">
        <label class="field ms-field">${this.#e("plan_name","Plan name")}
          <input
            ...
            autocomplete="off"
            .value=${t.name}
            @input=${i=>this.#t({type:"patch-plan-draft",patch:{name:f(i)}})}
          >
        </label>
        <div class="split plan-meta">
          <label class="field ms-field">${this.#e("plan_run_behavior","Cleaning order")}
            <select
              .value=${t.runBehavior}
              @change=${i=>this.#t({type:"patch-plan-draft",patch:{runBehavior:f(i)==="ordered"?"ordered":"intelligent"}})}
            >
        ...
        <div class="list" role="group" aria-labelledby="plan-rooms-heading">
          ${X(l,({room:i})=>i.roomId,({room:i,label:c,selected:d})=>{let v=d?t.rooms.findIndex(m=>m.roomId===i.roomId):-1;return n`
              <div class="room plan-room ms-row ms-row--stack" data-selected=${String(d)}>
                <div class="room-choice">
                  <label class="plan-room-label">
                  <input type="checkbox" .checked=${d} @change=${()=>this.#w(i.roomId)}>
                  ...
                  ${d?n`
                    <span>
                      ...
                      <button class="icon-button ms-btn ms-btn--icon" type="button" aria-label=${this.#e("move_room_down","Move {room} later",{room:c})} ?disabled=${v===t.rooms.length-1} @click=${m=>{m.preventDefault(),this.#h(v,1)}}>${R(U)}</button>
                    </span>
                  `:p}
                </div>
                ${d?n`
                  <div class="split room-settings">
                    <label class="field ms-field">${this.#e("v4_cleaning_system","Cleaning system")}
                      <select aria-label=${this.#e("v4_room_cleaning_system_named","Cleaning system for {room}",{room:c})} .value=${i.cleaningMode} @change=${m=>this.#l(v,{cleaningMode:f(m)})}>${I.map(m=>n`<option value=${m} ?selected=${m===i.cleaningMode} ?disabled=${!!("cadence"in i&&i.cadence?.mopEveryN&&m!=="vacuum")}>${this.#o(m)}</option>`)}</select>
                    </label>
                    <label class="field ms-field">${this.#e("cleaning_mode","Cleaning mode")}
                      <select aria-label=${this.#e("v4_room_cleaning_mode_named","Cleaning mode for {room}",{room:c})} .value=${i.coverageSetting} @change=${m=>this.#l(v,{coverageSetting:f(m)})}>${E.map(m=>n`<option value=${m} ?selected=${m===i.coverageSetting}>${this.#a(m)}</option>`)}</select>
                    </label>
                  </div>
                  ${this.#y(i,v,c)}
                `:p}
        ...
        <section class="stack" aria-labelledby="next-run-preview-heading">
          ...
          ${t.id?!b||!/^[0-9a-f]{64}$/u.test(b.previewToken??"")?n`<p class="problem" role="status">${this.#e("v4_next_run_preview_unavailable","A verified next-run preview is unavailable. Refresh the saved plan before starting it.")}</p>`:b.blocker?n`<p class="problem" role="alert">${this.#g(b.blocker)}</p>`:n`
                  ${t.dirty?n`<p class="notice" role="status">${this.#e("v4_next_run_preview_stale","This preview shows the saved plan. Save your edits to calculate the updated order and settings before starting.")}</p>`:p}
                  <ol class="list" aria-label=${this.#e("v4_next_run_preview_order","Next-run room order and effective settings")}>
                    ${b.rooms.map((i,c)=>{let d=1+b.missionBoundaries.filter(v=>v<=c).length;return n`<li class="ms-row ms-row--stack">
                        <span class="subtle">${this.#e("v4_next_run_mission","Mission {number}",{number:d})}</span>
                        <strong>${c+1}. ${i.name}</strong>
                        <span>${this.#o(i.cleaningMode)} \u00b7 ${this.#a(i.coverageSetting)}</span>
                        ${i.cadenceReasons.length?n`<small>${i.cadenceReasons.map(v=>this.#c(v)).join(" \xB7 ")}</small>`:p}
                      </li>`})}
                  </ol>
                `:n`<p class="subtle">${this.#e("v4_next_run_preview_save_first","Save this plan to calculate its exact room order and effective settings.")}</p>`}
        </section>
        ...
        <div class="plan-options" role="group" aria-labelledby="completion-heading">
          <label class="plan-option">
            <input type="checkbox" .checked=${t.returnToBase} @change=${i=>this.#t({type:"patch-plan-draft",patch:{returnToBase:k(i)}})}>
            <span class="plan-option-copy">
          ...
          <label class="plan-option">
            <input type="checkbox" .checked=${t.finishCurrentRoom} @change=${i=>this.#t({type:"patch-plan-draft",patch:{finishCurrentRoom:k(i)}})}>
            <span class="plan-option-copy">
          ...
          ${t.finishCurrentRoom?n`
            <label class="plan-threshold ms-field">
              ...
              <input type="range" min="0" max="100" step="5" .value=${String(t.finishCurrentRoomThreshold)} aria-label=${this.#e("plan_threshold","Minimum room progress")} @input=${i=>this.#t({type:"patch-plan-draft",patch:{finishCurrentRoomThreshold:Number(f(i))}})}>
            </label>
          `:p}
        </div>
        <div class="toolbar">
    ...
    `)}#R(){let e=this.state.resources.areas;return n`
      <div class="stack">
        ...
        ${this.state.draw.tool!=="outline"?n`<p class="subtle">${this.#e("v4_keyboard_draw_help","Keyboard: focus the map, use arrow keys to aim, then Enter to paint or erase at the crosshair. D selects Paint; E selects Erase.")}</p>`:p}
        ${this.#n(e.status,e.problem,n`
          <div class="group">
    ...
    `}#S(){let e=this.state.areaDraft,o=e.canRebind||e.status==="review",t=!o&&(e.status==="stale"||e.status==="unknown");return n`
      <div class="stack">
        ${o?n`<div class="notice" data-tone="warning" role="status">${this.#e("area_review_required","Review the saved outline on this current map, then confirm it.")}</div>`:p}
        ${t?n`<div class="problem" role="alert">${this.#e("area_redraw_required","This outline no longer matches the current room map. Redraw it before saving.")}</div>`:p}
        ...
        <div class="split">
          <label class="field ms-field">${this.#e("v4_cleaning_system","Cleaning system")}
            <select .value=${e.cleaningMode} @change=${a=>this.#t({type:"patch-area-draft",patch:{cleaningMode:f(a)}})}>${I.map(a=>n`<option value=${a} ?selected=${a===e.cleaningMode}>${this.#o(a)}</option>`)}</select>
          </label>
          <label class="field ms-field">${this.#e("cleaning_mode","Cleaning mode")}
            <select .value=${e.coverageSetting} @change=${a=>this.#t({type:"patch-area-draft",patch:{coverageSetting:f(a)}})}>${E.map(a=>n`<option value=${a} ?selected=${a===e.coverageSetting}>${this.#a(a)}</option>`)}</select>
          </label>
        </div>
        <div class="toolbar">
    ...
    `)}#v(e){try{return new Intl.DateTimeFormat(this.state.locale,{dateStyle:"medium",timeStyle:"short"}).format(new Date(e))}catch{return this.#e("v4_saved_map","Saved map")}}#u(){this.#i||customElements.get("matic-map-diagnostics-v4")||(this._diagnosticsLoadFailed=!1,this.#i=import("./diagnostics-panel-DXOFHT63.js").then(()=>{this.#i=null,this.requestUpdate()}).catch(()=>{this.#i=null,this._diagnosticsLoadFailed=!0}))}#E(){return customElements.get("matic-map-diagnostics-v4")?n`<matic-map-diagnostics-v4
        .state=${this.state}
        .localize=${this.localize}
        .disabled=${this.state.command!=="idle"&&this.state.command!=="failed"}
      ></matic-map-diagnostics-v4>`:this._diagnosticsLoadFailed?n`<div class="problem" role="alert">
